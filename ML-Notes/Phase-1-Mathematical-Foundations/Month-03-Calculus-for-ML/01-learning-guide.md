...
- Define the **gradient** and use it as the **direction of steepest ascent**; descent = \(-\nabla L\).
...
|---------|------------------------|---------------|
| **Derivative** | \(f'(x) = \lim_{h\to 0}\frac{f(x+h)-f(x)}{h}\); slope of tangent; rate of change | **Loss as function of one weight:** \(L(w)\); \(dL/dw\) tells you how much \(L\) changes when you nudge \(w\). |
| **Rules** | Power, product, quotient, chain; \((x^n)'=nx^{n-1}\), \((e^x)'=e^x\), \((\ln x)'=1/x\) | Used everywhere when deriving gradients (e.g., MSE, cross-entropy). |
...
| Concept | Definition / Intuition | ML Relevance |
|---------|------------------------|---------------|
| **Partial derivative** | \(\frac{\partial f}{\partial x}\): differentiate w.r.t. \(x\) while holding other variables constant | **How each parameter affects loss:** \(\partial L/\partial w_j\) = contribution of \(w_j\) to the loss. |
| **Notation** | \(\partial/\partial x\) vs \(d/dx\) when other variables exist | In ML we almost always have many parameters → partials. |
...
| Concept | Definition / Intuition | ML Relevance |
|---------|------------------------|---------------|
| **Gradient** | \(\nabla f = \bigl(\frac{\partial f}{\partial x_1},\ldots,\frac{\partial f}{\partial x_n}\bigr)^T\); vector of all partials | **Direction of steepest ascent**; magnitude = rate of increase in that direction. |
| **Descent** | \(-\nabla f\) is direction of **steepest descent** | We minimize loss → take steps in \(-\nabla L\). |

**Why it matters for ML:** The gradient \(\nabla L(\theta)\) is the single most important object in optimization: it tells you how to move all parameters at once to decrease the loss fastest.
...
| Concept | Definition / Intuition | ML Relevance |
|---------|------------------------|---------------|
| **Update rule** | \(\theta_{t+1} = \theta_t - \eta\,\nabla L(\theta_t)\); repeat until convergence | **THE core optimization algorithm** for training neural networks and most ML models. |
...
| Concept | Definition / Intuition | ML Relevance |
|---------|------------------------|---------------|
| **Learning rate \(\eta\)** | Step size: \(\theta_{new} = \theta - \eta\nabla L\) | **Too large:** divergence or oscillation. **Too small:** very slow convergence. |
| **Scheduling** | Decrease \(\eta\) over time (e.g., decay, step decay) | Often used in practice to stabilize and then refine. |

**Why it matters for ML:** Choice of \(\eta\) (and schedule) directly affects training stability and speed. It is one of the most important hyperparameters.
...
| Concept | Definition / Intuition | ML Relevance |
|---------|------------------------|---------------|
| **Single variable** | \((f\circ g)'(x) = f'(g(x))\,g'(x)\) | Composing small derivatives along a path. |
| **Multivariable** | \(\frac{\partial f}{\partial x} = \sum_k \frac{\partial f}{\partial z_k}\frac{\partial z_k}{\partial x}\) | **Backbone of backpropagation:** loss depends on output, output on hidden layer, hidden on weights → chain rule gives \(\partial L/\partial w\). |
...
| **Forward / backward** | Forward: compute output and store intermediates. Backward: compute \(\partial L/\partial\) for each node | PyTorch and TensorFlow “autograd” do exactly this. |
...
| Concept | Definition / Intuition | ML Relevance |
|---------|------------------------|---------------|
| **Stationary point** | \(\nabla f = \mathbf{0}\) | Candidates for minima (and maxima, saddle points). |
...
**Why it matters for ML:** We optimize loss \(L(\theta)\) over many parameters. Stationary points and curvature determine whether we get a good minimum or get stuck.
...
| Concept | Definition / Intuition | ML Relevance |
|---------|------------------------|---------------|
| **Convex function** | Bowl-shaped; one global minimum; no saddle points | Linear regression (MSE) is convex in \(\mathbf{w}\); gradient descent finds global min. |
...
|---------|------|---------------|
| **SGD** | One sample per update; \(\nabla L\) approximated by one example | **Practical optimization:** fast per step; noisy but often generalizes well. |
...
|-----|------|------------|
| 1–2 | Derivative as rate of change; rules (power, product, chain for 1D) | 3B1B “Derivative”; differentiate \(x^2\), \(e^x\), \(\ln x\) by hand |
| 3 | Partial derivatives; gradient vector | Imperial Week 1; compute \(\partial/\partial x\), \(\partial/\partial y\) for \(f(x,y)=x^2 y\) |
| 4 | Gradient = steepest ascent; \(-\nabla L\) = descent | 3B1B “Partial derivatives”; gradient of a 2D function |
| 5 | Gradient descent: \(\theta_{t+1} = \theta_t - \eta\nabla L\); implement in 1D/2D | Implement GD for a simple \(L(w)\); plot trajectory |
...
| 3 | Computation graph: forward pass (output), backward pass (gradients) | Draw graph: input → linear → loss; write \(\partial L/\partial w\) via chain rule |
...
| Day | Topic | Activities |
|-----|------|------------|
| 1–2 | Learning rate: too big vs too small; optional scheduling | Try \(\eta = 0.01, 0.1, 1.0\) on same problem; plot loss curves |
...
| 4 | Phase 1 wrap-up: link calculus to linear algebra (Month 1) and MLE (Month 2) | Summarize: loss from MLE; gradients via chain rule; parameters in \(\mathbb{R}^n\) |
...
| **Chain rule (1D)** | \((f\circ g)'(x) = f'(g(x))\,g'(x)\) |
| **Chain rule (multivariable)** | \(\frac{\partial f}{\partial x} = \sum_k \frac{\partial f}{\partial z_k}\frac{\partial z_k}{\partial x}\) |
| **MSE gradient (one weight)** | \(\frac{\partial}{\partial w}\frac{1}{2}(wx - y)^2 = (wx - y)x\) |
...
1. **Loss surface:** Choose a 2D function (e.g., paraboloid \(L(w_1,w_2) = w_1^2 + w_2^2\), or a saddle). Create a grid of \((w_1, w_2)\) and compute \(L\). Plot as a **contour plot** or **surface plot** (e.g., `matplotlib`: `contour`, `contourf`, or `plot_surface`).
2. **Gradient descent:** Implement the update \(\theta_{t+1} = \theta_t - \eta\nabla L(\theta_t)\). Store the sequence of \(\theta\) (e.g., list of points).
3. **Plot path:** Overlay the sequence of \((\theta_1, \theta_2)\) on the contour plot (e.g., scatter or line). Mark start and end.
...
5. **Learning rate:** Try different \(\eta\) (e.g., 0.01, 0.1, 0.5) and show how too large \(\eta\) causes oscillation or divergence, and too small causes slow convergence.
...
1. What is the geometric interpretation of the gradient? Why do we use \(-\nabla L\) for minimization?
2. Write the gradient descent update rule in one line. What is the role of \(\eta\)?
